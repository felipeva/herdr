...
impl App {
    fn finish_worktree_list(
        &self,
        id: String,
        source: WorktreeSource,
        entries: Vec<crate::worktree::ExistingWorktree>,
    ) -> String {
    ...
    fn finish_worktree_open(
        ...
        let requested_parent_id = source
            .workspace_idx
    ...
    fn mark_worktree_membership(
        ...
        let parent_workspace_id = if target_is_linked_worktree {
            // Reopening a worktree from another parent must not steal it from a live parent.
            let has_recorded_parent = self.state.workspaces[target_ws_idx]
                .worktree_space()
                .is_some_and(|space| {
                    space.is_linked_worktree && space.parent_workspace_id.is_some()
                });
            has_recorded_parent
                .then(|| self.state.worktree_parent_idx(target_ws_idx))
                .flatten()
                .map(|idx| self.public_workspace_id(idx))
                .or(requested_parent_id)
        } else {
            None
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn api_worktree_create_records_requested_parent_among_same_repo_workspaces() {
        let repo = create_committed_repo("api-worktree-parent-repo");
        let worktree_root = unique_temp_path("api-worktree-parent-root");
        ...
        app.state.selected = 0;
        app.state.worktree_directory = worktree_root.clone();
        let a_id = app.state.workspaces[0].id.clone();
        let c_id = app.state.workspaces[2].id.clone();

        let create_from = |app: &mut App, workspace_id: &str, branch: &str| {
            let (respond_to, response_rx) = response_channel();
            let request = Request {
                id: "req".into(),
                method: crate::api::schema::Method::WorktreeCreate(WorktreeCreateParams {
            ...
            assert!(app.handle_deferred_worktree_api_request(request, respond_to, false));
            // Earlier worktree panes emit their own events before this completion arrives.
            let response = loop {
        ...
        let (a_child, a_worktree) = create_from(&mut app, &a_id, "worktree/from-a");
        let (c_child, c_worktree) = create_from(&mut app, &c_id, "worktree/from-c");

        assert_eq!(
            a_child.worktree.unwrap().parent_workspace_id.as_deref(),
            Some(a_id.as_str())
        );
        assert_eq!(
            c_child.worktree.unwrap().parent_workspace_id.as_deref(),
            Some(c_id.as_str())
        );
        let c_child_idx = app.parse_workspace_id(&c_child.workspace_id).unwrap();
        assert_eq!(app.state.worktree_parent_idx(c_child_idx), Some(2));

        for (_, runtime) in app.terminal_runtimes.drain() {
            runtime.shutdown();
        }
        for worktree in [a_worktree, c_worktree] {
            let remove = crate::worktree::build_worktree_remove_command(
                &repo,
                Path::new(&worktree.path),
                false,
    ...
    async fn deferred_api_worktree_create_checks_out_existing_branch() {
        ...
        assert!(app.handle_deferred_worktree_api_request(request(), response_tx, false));
        let event = wait_for_app_event(&mut app);
        app.handle_internal_event(event);
        let response = response_rx
            .recv_timeout(std::time::Duration::from_secs(2))
    ...
    fn deferred_api_worktree_create_failure_clears_pending_checkout() {
        ...
        assert!(app.handle_deferred_worktree_api_request(request(), first_tx, false));
        let event = wait_for_app_event(&mut app);
        app.handle_internal_event(event);
    ...
    async fn deferred_api_worktree_create_completes_after_source_workspace_changes() {
        ...
        assert_eq!(app.state.worktree_parent_idx(2), Some(1));
    ...
    #[tokio::test]
    async fn deferred_api_worktree_create_does_not_reassign_changed_source_to_sibling_parent() {
        let mut app = test_app_with_event_hub(crate::api::EventHub::default());
        let repo = create_committed_repo("api-worktree-create-sibling-parent-repo");
        let checkout = unique_temp_path("api-worktree-create-sibling-parent-checkout");
        std::fs::create_dir_all(&checkout).unwrap();
        let checkout_key = crate::worktree::canonical_or_original(&checkout);
        let membership = |key: &str| crate::workspace::WorktreeSpaceMembership {
            key: key.into(),
            label: "herdr".into(),
        ...
        let mut sibling = Workspace::test_new("a");
        sibling.worktree_space = Some(membership("repo-key"));
        let mut source = Workspace::test_new("c");
        source.worktree_space = Some(membership("other-key"));
        let source_id = source.id.clone();
        app.state.workspaces = vec![sibling, source];
        app.pending_api_worktree_creates
            .insert(checkout_key.clone(), 9);
        let (respond_to, response_rx) = response_channel();
        ...
        assert_eq!(
            app.state.workspaces[2]
                .worktree_space()
                .and_then(|membership| membership.parent_workspace_id.as_deref()),
            Some(source_id.as_str())
        );
        assert_eq!(app.state.worktree_parent_idx(2), None);
    ...
    #[test]
    fn reopening_worktree_keeps_only_a_live_recorded_parent() {
        let mut app = test_app_with_event_hub(crate::api::EventHub::default());
        app.state.workspaces = ["a", "c", "child"]
            .into_iter()
            .map(Workspace::test_new)
            .collect();
        let a_id = app.state.workspaces[0].id.clone();
        let c_id = app.state.workspaces[1].id.clone();
        ...
        for idx in 0..2 {
            app.state.workspaces[idx].worktree_space = Some(membership("/repo/herdr", false, None));
        }
        let source = WorktreeSource {
        ...
        let recorded_parent = |app: &App| {
            app.state.workspaces[2]
                .worktree_space()
                .and_then(|membership| membership.parent_workspace_id.clone())
        };

        for (existing_parent, expected_parent) in [
            (Some(a_id.clone()), a_id.clone()),
            (None, c_id.clone()),
            (Some("closed-parent".to_owned()), c_id.clone()),
        ] {
            app.state.workspaces[2].worktree_space =
                Some(membership("/worktrees/child", true, existing_parent));
            app.mark_worktree_membership(
                &source,
                2,
                "/worktrees/child".into(),
                true,
                Some(c_id.clone()),
                false,
            );
            assert_eq!(recorded_parent(&app), Some(expected_parent));
        }
    }
    ...
    fn api_worktree_open_preserves_explicit_membership_after_shell_cd() {
        ...
        app.mark_worktree_membership(&source, 0, repo.clone(), false, None, false);
    ...
    async fn deferred_worktree_open_rechecks_source_identity_and_existing_target() {
        ...
        for source_change in ["reordered", "removed", "repurposed"] {
            let mut app = app_with_parent(&repo);
            let source_id = app.state.workspaces[0].id.clone();
            let (respond_to, response_rx) = response_channel();
            assert!(app.handle_deferred_worktree_api_request(
                Request {
                    id: "pending-open".into(),
                    method: crate::api::schema::Method::WorktreeOpen(WorktreeOpenParams {
                        workspace_id: Some(source_id.clone()),
            ...
            let completed_discovery = wait_for_app_event(&mut app);
            assert!(response_rx.try_recv().is_err());
            assert!(app.state.workspaces[0].worktree_space().is_none());

            let original_source = app.state.workspaces.remove(0);
            let mut unrelated = Workspace::test_new("unrelated");
            unrelated.identity_cwd = other_repo.clone();
            if source_change == "repurposed" {
                unrelated.id = source_id.clone();
            }
            let mut opened_target = Workspace::test_new("opened-during-discovery");
            opened_target.identity_cwd = checkout.clone();
            let target_id = opened_target.id.clone();
            app.state.workspaces = vec![unrelated, opened_target];
            if source_change == "reordered" {
                app.state.workspaces.push(original_source);
            }
            app.state.terminals.clear();
            app.state.ensure_test_terminals();
            app.state.active = Some(0);
            app.state.selected = 0;
            app.handle_internal_event(completed_discovery);
            ...
            assert!(
                already_open,
                "target opened during discovery must be reused"
            );
            assert_eq!(workspace.workspace_id, target_id);
            assert!(
                app.state.workspaces[0].worktree_space().is_none(),
                "{source_change} source must not mark an unrelated workspace"
            );
            assert_eq!(
                app.state.active,
                Some(0),
                "background open must preserve focus"
            );
            assert_eq!(app.state.workspaces.len(), 3);
            assert_eq!(
                crate::worktree::canonical_or_original(
                    &app.state.workspaces[2].worktree_space().unwrap().repo_root
                ),
