...
| 会话 | `session.snapshot` |
| 工作区 | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.move`、`workspace.move_block`、`workspace.move_group`、`workspace.report_metadata`、`workspace.close` |
...
`workspace.move_group` 会将 `workspace_id` 连同其作为父工作区的关联 worktree 一起移动到 `before_workspace_id` 之前,或移动到末尾。响应与 `workspace.move_block` 相同。
