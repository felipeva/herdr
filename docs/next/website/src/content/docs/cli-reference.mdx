...
`workspace move` places a workspace before another workspace or at the end of the list. Moving a worktree parent also moves its linked worktrees.
...
Worktrees are normal Herdr workspaces with Git checkout provenance. `worktree create` creates a Git worktree checkout, opens it as a workspace, and groups it under the source workspace. When several workspaces share one repo, pass `--workspace` to choose the parent. Without `--workspace` or `--cwd`, Herdr uses the focused workspace. If `--branch` names an existing local branch, Herdr checks it out; otherwise it creates the branch from `--base` or `HEAD`. Without `--path`, Herdr creates the checkout under `<worktrees.directory>/<repo>/<branch-slug>`.
